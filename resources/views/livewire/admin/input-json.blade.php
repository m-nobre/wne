...
<div>
    ...
    <div class="relative flex h-10 w-full min-w-full max-w-full">
        ...
        <div class="space-y-8 divide-y divide-gray-200"
            x-data='{
                json{{ucfirst($model)}}nameSelected(e) {
                    console.log(e)
                    let value = e.target.value
                    let id = document.body.querySelector("#json{{ucfirst($model)}}nameOptions [value=\""+value+"\"]").dataset.value

                    $wire.dispatchSelf("json{{ucfirst($model)}}nameSelected", {author_id: id});
            ...
            >
                <input
                    type="text"
                    list="json{{ucfirst($model)}}nameOptions"
                    id="json{{ucfirst($model)}}nameInput"
                    ...
                    x-on:change.debounce="json{{ucfirst($model)}}nameSelected($event)"
                >
                @if ($newPerson)
                ...
                <datalist id="json{{ucfirst($model)}}nameOptions">
